...
### 5.3 当前处置
已 `git restore` 回退上述 6 个跟踪文件，并删除 4 个新增文件。**工作区已与 `6383c24` 一致**，
`node tools/regression.mjs` 回到 33/33，`node tools/e2e-sim.mjs` 应通过。
**唯一遗留**：当前磁盘上的 `WavePaint.exe`（约 00:18 构建）仍内嵌着第五轮坏 JS，必须重建（见 §9）。
本接手会话的安全策略禁用了 `csc.exe`，所以我无法在此重建 exe——请你在本机/允许的环境跑 `.\build.ps1`。
...
## 7. 待办功能需求清单（按优先级，含推荐实现路径）

> 以下全部尚未完成。请逐个做、做完一个验证一个、单独 git 提交 + 重建 exe。
...
3. 任何 js/css/img 改动后：**必须重建 exe**：在本机 PowerShell 跑
   `cd WavePaintSim; .\build.ps1`（内部 `csc` 编译 Launcher 并内嵌资源）。
   若环境禁用 csc，请换一个允许的环境构建，或明确告知用户由其手动构建——**未重建 exe 的改动对用户不可见**。
4. 验证门槛（缺一不可）：
