...
- **核心右键菜单自带 10/16 进制转换项** —— 不要自绘、也不要往 `.context-menu` 注入：
  注入会被用户否决（分隔线风格不符、菜单已开时再右键注入区闪烁、功能重复冲突）。
  自绘菜单会与核心菜单并存（双菜单），拦截会删掉用户需要的原功能。都试过，都被否了。
- 我们的职责只有一件事：**保证转换结果正确** = `patchCoreValueToLabel`
  （已补丁实例 + 整条原型链）+ `refreshBusLabels`（按信号自身 radix）。
- **混淆核心字符串是拆分的**，grep 完整名搜不到；用片段法（如 'context-me'+'nu'）。
- 验证进制必须用 **≥10 的值**（255 → FF）：0~9 时 hex 与 dec 显示天然相同。
- **媒体查询里的 CSS 规则也要查**（白条真凶在 `@media (max-width:1200px)` 里）。
- 构建前先确认 WavePaint.exe 没在运行（文件锁会让编译失败且脚本仍报 exit 0）。

## 九、用户长期约定（2026-09-01 确认，所有 AI 照此执行）

- **构建前直接结束 WavePaint.exe 进程，无需再向用户确认**：
  `taskkill /F /IM WavePaint.exe`（Git Bash 里参数是 `/F /IM`，`//IM` 会报错）。
