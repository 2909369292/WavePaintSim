...
> `REQUIREMENTS.md`（需求清单）与 `2026-08-30.md`~当日日志、`docs/`（设计/重构细节）。
> 本文件自包含：关键信息不用跳读也能承接项目。
>
> ⭐ **2026-09-08 起最高优先入口 = 同目录 `PROJECT-MASTER.md`（项目总纲）**：
> 项目介绍/思路/全部约束与做法/需求台账/路线图/**进行中交接任务**都在那里，
> 本文件保留为铁律细节速查。新对话请先读总纲第六章（进行中/交接）。
