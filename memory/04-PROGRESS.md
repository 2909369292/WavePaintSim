...
| 最近完成文档 | #80 记忆体系重构（旧目录已删除）+ **第十一轮规划重排**（2026-09-09，纯文档）：近期主线改 #86 → #76，#87 优先级提高，侧栏/整体 UI 重构设计提级，#84/#77 推迟远期（见 §4.9 / 08-ROADMAP） |
| 最近一轮（第十一轮，规划重排） | 用户拍板优先级：先完成 **#86 → #76**（重点：底层支持更多 Verilog 代码互动 + 代码解析能力；交互层仿照 Verdi 规划）；**#87 优先级提高**；**侧栏/整体 UI 重构设计优先级提高**；**#84 波形查看增强、#77 Active Annotation 推迟打入远期**。纯文档轮（只改 memory/），未动代码无需重建 exe；记忆同步 + commit |
| 当前阻塞 | 无 |
| 下一步主线 | 开工 **#86**（实例→模块定义源码跳转 + 源码文件导入 + 例化解析增强，拆解 A1~A4 见 08 §1.1）→ **#76**（模块体内符号索引/scope 映射 + RTL 树点行加波形 + 代码点变量加波形 + 双向跳转 + 信号组入 `.wp`，拆解 B1~B5 见 08 §1.2）→ **#87①/②**；#84/#77 已推迟远期（08 §3） |
