...
### 4.27 ✅ 已完成：**回归真机基线「四区停靠 UI」全部落地 + 浮窗/菜单真 bug 修复 + 无死局护栏 + exe 重建**（第二十九~三十五轮，2026-09-13；代码 commit `aec17e8`）

**用户原话（本轮全部需求的唯一来源）**：
> 「经测试，**别的 Agent 写的 UI 不可靠**，请还是按照**原来基线原来的 UI** 进行修改，修改 bug 逐步完善并实现。……
> 实现**左中右下分块的模式，不同块的界面可以相互拖拽**，布局和 **Verdi** 一样，其中最重要的**中间的波形区需要特殊一些，
> 波形区的显示直接套用原本波形区显示的代码，要求显示完全一样**，然后**之前的编辑栏位置不变**，
> **所有的分栏、所有的块儿都在之前编辑栏的下边**。……之前最初版 UI **功能上没问题但有很多 bug**（如窗口拖动），
> **尽量采用成熟的 UI 方案或者 UI 套件**也可以来解决此问题。**保证 UI 的稳定性**。」

**判定**：
1. 第二十八轮「把 UI 外包给外部 AI Agent」的路线**作废**（用户实测不可靠）→ 全部 UI 改动**回归真机基线**，直接落在产品文件上（见 07 **D28**）。
2. 「尽量采用成熟 UI 方案/套件」在**零构建约束**（07 D22：禁 React/Vue/打包器）下的落地方式 = **自研停靠引擎 + 严格断言护栏**，
   用「宿主节点搬家 + 预览==落位单一数据源 + 拖拽韧性三件套 + 无死局护栏 + 真实 Edge 断言」换取稳定性。

#### ① 外壳与布局（`index.html` + `css/workspace.css`，20,024 B）

- **`#menu-bar` 与 `#toolbar`（=「编辑栏」）原样吸顶不动** → 用户口径「编辑栏位置不变」**逐字落实**；
  新建 **`#workbench`** 挂在两者**之下** → 口径「所有的分栏、所有的块儿都在之前编辑栏的下边」**逐字落实**。
- **四区分块**（默认预设 `presetSim()`，全部在 `#workbench` 内）：

```
左 ≈19%  [rtl / vcd]        中 ≈55%  [wave]        右 ≈26%  [source / tb]
下 整行   [console]（split('col',[0.74,0.26]) 的第二行）
```

- 底部新增 `#wp-status-bar`（纯只读：布局名 + 可见面板数 + 版本戳）；**不放任何菜单/交互**（恢复入口在页签条 `＋`，见 07 D33）。
- **地基规则（勿删）**：`body.wp-dock{display:flex;flex-direction:column;height:100vh;overflow:hidden}` —— 缺它 `#workbench` 会塌成 19px（06 **P40**）。
- 实测几何（`tools/dock-probe.mjs` §1，Emulation 1680×980）：`#workbench={x:0,y:86,w:1680,h:868}`、`#main-area={x:328,y:125,w:908,h:594}`、`#wp-status-bar={y:954,h:26}`、3 条分隔条。

#### ② 面板内容 = 真机节点**整块搬家**（不克隆、不重绘、不复制 id）

| 面板 | 宿主节点（真机原有） | 说明 |
|---|---|---|
| `wave` | `#main-area`（含 `#wave-view` / `#wave-canvas`） | **波形显示区原样搬入；绘制代码 `js/wavepaint.clean.js` 一行未改 → 显示 1:1** |
| `source` | `#sim-card-source` | Verilog/SV 源码（CodeMirror 6 宿主 `#verilog-cm-host`） |
| `rtl` | `#sim-card-rtl` | RTL 结构树（纯代码层级浏览，无接口信号，第十二轮口径） |
| `vcd` | `#sim-card-vcd` | VCD 信号层次 |
| `tb` | `#sim-card-tb` | Testbench（只读，G1 未解，`#95` 暂不实现） |
| `console` | `#sim-console`（**新建容器**，内含原址搬入的 `#sim-console-log` → `#sim-status` → `#sim-recover`） | 仿真状态/日志 + 自愈按钮 |

- **搬家规则**：只 `appendChild` 换父容器，**绝不 `remove()`**；未显示的面板摘到 `#mk-park`（`display:none !important`）；
  面板 body 内始终只有**一份**宿主节点 → 六个冻结 id 全文档仍唯一。
- `#sim-panel` / `#sim-panel-body` / `#sim-resize-x` **原地保留（仅 CSS 隐藏）** → `ui-bridge.initRefs()` 拿到的引用非空，旧功能零回归。
- **波形容器变宽后重绘**：核心 `drawWaveform()` **不监听 resize**（06 P41）→ 引擎 `redrawWave()` 调 `window.__wpf.scheduleRedraw()`，在 `afterLayout()` 的 rAF 与 60ms 定时器各调一次；实测画布随面板宽重算（908 → 1666）。

#### ③ 交互能力（全部真机可用）

| 手势 | 行为 |
|---|---|
| 拖动**页签** | 停靠：落入某组页签条 = **并入**（成为该组新页签）；落在组左/右/上/下 = **分栏**；落到工作区**外缘环带** = **新建整行/整列** |
| 双击 / 中键页签 | **浮出**为页内浮窗 `.mk-float`（可拖动、可缩放、可再拖回停靠） |
| 拖**分隔条** | 改相邻两格比例；只影响相邻两格 |
| `✕` | **隐藏**该面板（不是关闭）；只剩 1 个可见面板时**拒绝**并给中文提示（护栏） |
| 页签条右侧 `＋` | 打开**面板/预设菜单**（10 项 = 6 面板 + 3 预设 + 恢复默认布局） |

- **预览 == 落位（同一数据源）**：`computeDrop(x,y,panelId,strict)` 在**布局树副本**上真跑 `applyOp()` → `layoutRects()` 反算矩形；
  `showIndicator()` 只消费 `d.rect`；`placePanel()` 执行**同一个操作** → 不可能出现「预览一套、落位一套」。
- **拖拽韧性三件套**（07 **D30**）：`DRAG_THRESHOLD=4`（区分点击/拖拽）+ 四种拖拽全 `setPointerCapture` + `pointermove/up/cancel` 挂 `window` + `activeDragCancels`/`cancelAllDrags()`（`Esc`、`window blur`、`pointercancel` 三条路径都取消且**不提交**）→ 无「拖拽僵尸态」（06 **P42**）。
- **落点优先级 = 页签条 > 外缘环带**，唯一例外是顶边最外 `DROP.outerTop=8` 窄带仍判「新建整行」（07 **D31**；常量表 `DROP={outer:14,outerTop:8,tabBand:3,groupRatio:0.25,groupMin:28,groupMax:96,splitShare:0.30,rootShare:0.22}`）。
- **浮窗拖动走 strict 落点**（07 **D32**）：只认工作区外缘环带与组页签条，**组矩形内部「追加」判定全部失效** → 停在空白处 = 保持浮窗 = **真正能自由摆放**。

#### ④ 本轮修掉的三条真 bug（全部「实测复现 → 修复 → 加断言看守」）

1. **浮窗永远被吸进组、无法自由摆放**（06 **P44** / 07 **D32**）：根因 = 浮窗复用了停靠落点判定，而工作区绝大部分面积被某个组的矩形覆盖 → 松手必被吸收。修法 = `computeDrop(..., strict=true)`。
2. **`render()` 清根残留 → 「四区变六区」**（06 **P46** / 07 **D34**）：根因 = 清理语句按「根一定是 `.mk-split`」写死；布局树塌成单 `tabs` 节点时顶层插的是 `.mk-group` → **孤儿组永不被删**，隐藏到只剩 1 个面板再 `reset()` 后 `.mk-group` 计数 = 6（应 4）。修法 = 根节点打标 `data-mk-root` + 清理改 `:scope > [data-mk-root]`。**该 bug 由探针 §8 首跑 FAIL 先于用户发现**。
3. **`✕` 无护栏 → 空工作区死局**（06 **P45** / 07 **D33**）：修法 = `✕` 语义定为隐藏 + `hidePanel()` 拒绝删最后一个 + 页签条 `＋` 菜单作为唯一恢复入口 + `restore()` 读到旧版零面板数据时兜底回 `presetSim()`。

- 同批修正的判定顺序问题：**外缘环带与页签条几何重叠导致「并入」被顶掉**（06 **P43** / 07 **D31**）。

#### ⑤ 测试与工具（本批新增/改造）

- **新增 `tools/dock-probe.mjs`（46,500 B）** = 停靠 UI 专用回归：内部起 `dev-server` + **真实 Edge headless + CDP**（Emulation 1680×980）；
  分区：**§1** 外壳/四区几何（含 `#workbench`/`#main-area`/`#wp-status-bar` 矩形与 3 条分隔条）、**§2** 预览==落位、**§2.5** 阈值/Esc/失焦、**§3** 分隔条、**§4** 浮出/预设/持久化（键 `wavepaint.workspace.layout.v1`）、**§5** 画布随面板宽重算、**§6a~6e** 更多拖动路径、**§7** 浮窗拖动/缩放（16 项）、**§8** 面板菜单/隐藏护栏（8 项）；末行 `✔ 全部通过`。产物截图 `.e2e-tmp/dock-1-default.png` ~ `dock-7-*.png`。
- **`tools/e2e-ui.mjs` 改走 `?dock=off`** → 旧右侧栏口径的 **88/88** 原样保住（逃生口见 07 D29）。
- **`tools/gen-mock-page.mjs` 加弃用护栏**：默认只打印弃用提示并 `exit(0)`（因为真机 `index.html` 现已自带 `#workbench`/`#mk-*`，再追加旧 `prototype/mock-tail.html` 会产出**重复 id 的坏页**）；确需考古用 `--force`。

#### ⑥ exe 重建与真机冒烟（C1 触发 → 已按 C 线执行）

- `.\build.ps1` → `csc exit: 0`、`res count: 54`（新增 `css/workspace.css` + `js/sim/dock/workspace.js` 自动内嵌，`tools/gen-resources.mjs` 递归收集）。
- **`WavePaintClean.exe` = 22,096,896 B**；`version.txt` = **`v0.4.0 build 2026-09-13 22:36:31 aec17e8`**。
- **C8 特征串核验**：`至少保留一个面板`=1、`mk-grp-btns`=6、`wavepaint.workspace.layout.v1`=1、`data-mk-root`=2（全 > 0）。
- **真机 exe 冒烟**（C15 正确前置：先拿到新进程端口再跑探针）：`node tools/exe-smoke.mjs` 全过
  （`{"core":"object","wpf":"object","doc":"object","canvas":true,"zh":true}`、弹窗「添加时钟信号 / 时钟名称：/ 确定」、**异常：无**）。
- **停靠外壳专项**（临时脚本，未入库）：`{"dock":true,"hasEngine":true,"groups":4,"tabs":["rtl|vcd","wave","source|tb","console"],"wb":[0,86,1680,868],"wave":[328,125,908,594],"status":[954,26],"version":"v0.4.0 build 2026-09-13 22:36:31 aec17e8","canvasBlank":"1540x600","plusBtn":true}`、控制台异常无。

#### ⑦ 本轮实测（全部实跑）

| 套件 | 结果 |
|---|---|
| `node tools/dock-probe.mjs 8976` | **✔ 全部通过**（§1~§8；reset 后 `{"groups":4,"vis":6,"hidden":0}`；隐藏到 1 个时 `{"vis":["wave"],"groups":1,"hidden":[5 项]}`；菜单 10 项） |
| `node tools/regression.mjs` | **79/79** |
| `node tools/e2e-sim.mjs` | **失败 0 项** |
| `node tools/e2e-rtl.mjs` | **61/61**（资源加载失败 0、控制台异常无） |
| `node tools/e2e-ui.mjs` | **88/88**（`?dock=off` 旧口径） |

#### ⑧ 红线与冻结契约

- **24 个冻结 id 一个未改**（`sim-panel` / `source-files` / `verilog-source` / `rtl-tree` / `vcd-tree` / `sim-status` / `sim-recover` / `main-area` / `menu-bar` / `toolbar` / `app-version` … 全清单见 09 §1 与 08 §2.5）。
- **`js/wavepaint.clean.js` 与 `js/sim/engine.js` 一行未动**（**C9** 守住）→ 波形显示区 1:1 的根本保证。
- `index.html` 只做 4 类改动：① `</style>` 后插 `css/workspace.css`；② `#sim-status`/`#sim-recover` 原址搬进新 `#sim-console`；③ `#sim-panel` 之后插 `#workbench` + `#wp-status-bar`；④ `js/sim/ui-bridge.js` 之前插普通 `<script src="js/sim/dock/workspace.js">`（**必须早于 ESM**）。
- `js/sim/ui-bridge.js` 只两处改动：`initPanelLayout()` 入口加 `if (window.__wpDock) return;`；`showAppVersion()` 双写 `#app-version` + `#wp-version`。

#### ⑨ 改动文件（commit `aec17e8`，7 files / +2275 −11）

| 文件 | 改动 |
|---|---|
| `js/sim/dock/workspace.js` | **新增 1,169 行**（自研停靠引擎：布局树/预设/拖拽/浮窗/菜单/持久化/护栏） |
| `css/workspace.css` | **新增 20,024 B / 309 行**（全部规则挂 `body.wp-dock` 之下；含 `?dock=off` 兼容垫片） |
| `index.html` | +65 −11（外壳 `#workbench`/`#wp-status-bar`、`#sim-console` 容器、样式与脚本引用） |
| `tools/dock-probe.mjs` | **新增 707 行**（真实 Edge 停靠回归） |
| `js/sim/ui-bridge.js` | +14 −? （两处小改，见 ⑧） |
| `tools/e2e-ui.mjs` | 改走 `?dock=off` |
| `tools/gen-mock-page.mjs` | 加弃用护栏 |

#### ⑩ 遗留 / 下一步

- **待用户 review**：双击 `D:\Files\Code\波形\WavePaintClean.exe`（`#wp-version` 自查 `v0.4.0 build 2026-09-13 22:36:31 aec17e8`）。
- **D1 剩余** → D2（浮窗最大化/键盘/层级）→ D3（持久化扩展 + 3 预设收口，**不写 `.wp`**）→ D4 + U3（真机工具带分组/刻度/尺寸/溢出，修 **P-UI-02** 真机 1440/1280 越界）。
- **可选淘汰（待用户确认）**：`prototype/**`（4 文件）、`WavePaintMockup.exe`、`build-prototype.ps1`、`tools/mock-probe.mjs`、`tools/mockup-exe-smoke.mjs`、`tools/mock-probe` 相关、`docs/ui-agent/**`。**未删，等用户点头。**
- **#95（G1~G8）** 仍由用户明确「暂不实现」；**#84 / #77 / #78** 远期（08 §3）。
