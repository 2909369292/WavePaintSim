...
> **【状态更新 · 第二十九~三十五轮 2026-09-13】本节的「三区 + 一条控制带」草案已被 `§6` 的 D 线实现取代**：
> 真机已落地 **四区停靠 UI**（LEFT `[rtl/vcd]` / CENTER `[wave]` / RIGHT `[source/tb]` / BOTTOM `[console]`），
> 依据 07 **D28~D34** 与 04 §4.27。下文保留作为**历史设计依据**（**上文四条红线仍然全部有效、未变**）。
...
| 项 | 内容 | 说明 |
|---|---|---|
**⏳ 剩余 → 【第二十九~三十五轮 2026-09-13 更新：以下三项已随四区停靠 UI 落地 / 改变形态】**

| 项 | 内容 | 状态（第二十九~三十五轮 2026-09-13） |
|---|---|---|
| VCD 树移入波形区 | 把 VCD 树从侧栏卡片移到波形区（原 P1 内容） | **改为独立面板 `vcd`**，默认与 `rtl` 同组落 **LEFT**（`tabs(['rtl','vcd'],'rtl')`）；未直接并入波形区 |
| TB 控制带收敛 | 把 4 卡进一步收敛为「一条控制带 + 折叠 TB」 | **形态已变**：TB 成为独立面板 `tb`（`#sim-card-tb` 整块搬家），默认落 RIGHT 与 `source` 同组；未单独成控制带 |
| 层次树左置 | `#rtl-tree` 移到左侧独立区（原「选项 B 位置」） | **✅ 已落地**：`rtl` 面板默认落 **LEFT**（用户第二十三轮已解除该限制；第二十九~三十五轮实现） |
...
> **【落地状态 · 第二十九~三十五轮 2026-09-13】四条结论已全部实现在真机**：
> ① 完整四区 dock（LEFT `[rtl/vcd]` / CENTER `[wave]` / RIGHT `[source/tb]` / BOTTOM `[console]`）；
> ② RTL / VCD 树默认落 **LEFT**；③ 浮出 = **页内浮动面板**（不碰 OS 窗口）；
...
- **【第二十九~三十五轮新增】四区停靠 UI 已落地真机，勿回退到外部 Agent 路线**（07 D28）：`js/sim/dock/workspace.js`
  + `css/workspace.css` + `index.html` 外壳（`#workbench` / `#wp-status-bar`）是唯一实现；`prototype/**` 与
  `docs/ui-agent/**` 已**弃用**（未删，等用户点头）。面板节点一律**整块搬家不克隆**（`#main-area` / `#sim-card-*` /
  `#sim-console`），`#sim-panel` **原地保留（仅 CSS 隐藏）** —— 破坏这两条会直接打断 `ui-bridge.initRefs()` 的非空引用。
- **【第二十九~三十五轮新增】`render()` 必须按 `[data-mk-root]` 清根**（07 D34 / 06 P46）——按 `.mk-split` 清会在布局树
  塌成单 `tabs` 时遗留孤儿组，实测「**四区变六区**」。
...
## 8. 大功能方向研讨（第五十四轮 2026-09-15 立；**只谈大方向，不谈细小优化**）

> 用户口径：后续规划**只讨论大功能 / 大方向**，细化项一律不做。以下为候选方向 + 推荐优先级，
> **尚未逐项落地**，实施前需与用户逐条确认。用户长期口径不变：**加信号靠「在代码里点变量名」**，
> RTL 树**只用于看代码层级**，不在树上展示端口/接口信号。

**现役技术底座**（写新方案时必须先认这个基线）：仿真内核 = **Icarus Verilog `iverilog -g2012` + `vvp`**
（随 exe 内嵌 `ivl.zip` 解压到临时目录，见 `WavePaintLauncher.cs`）；波形来源 = 后端回传 `@@VCD:` 文本段
（见 `07 D36`）；本地读盘通道 = `api/fs` / `api/pick`（见 `07 D57`）。

### 8.1 推荐优先级（Top 5，按「用户价值 ÷ 改造成本」排序）

1. **交叉联动（cross-probing / Active Annotation）** —— 三向打通「源码 ↔ 波形 ↔ 结构树」：
   源码里点变量 → 加波形 / 高亮已有波形；波形上点某时刻的某信号 → 跳源码定义行 + 跳到该赋值点；
   结构树点实例 → 跳源码。**这是「像 Verdi」的第一性功能**，比任何 UI 细节都更能拉开差距。
   前置：源码符号表（信号 ↔ 定义位置 ↔ 层次路径）+ 波形元数据（每个 VCD 变量对应的层次路径）。
2. **编译 / elaboration 前端完整对齐 Verdi 语义** —— 现在只有 `-g2012 -s tb` + 搜索根凑文件；
   大方向 = 真正的「**文件表 + 库 + 宏**」前端：`-f` filelist、`+incdir+`、`-y` / `+libext+`（本轮已部分对齐）、
   `-D` define、多库 / 多配置（`config`）、`+define+` 优先级；并把 iverilog 的报错**解析成可点击的源码行**
   （file:line:col → 编辑器跳转 + 行内标记）。**没有它，稍微正经一点的工程都跑不起来。**
3. **波形数据层独立化（格式 + 规模）** —— 把「波形数据」从「后端回传文本」抽成独立层：
   支持 **VCD / FST**（可读），大文件**分块 / 流式**加载（现在整段文本一次性推进浏览器，几十 MB 就会卡死），
   多 session 并存、波形文件另存 / 打开 / 对比（用户已要求系统级打开保存对话框，语义已就绪）。
4. **仿真会话与多顶层** —— 一个应用内**多个仿真会话**（不同 top / 不同参数 / 不同编译选项）并存并可切换，
   每个会话独立 waveform + 日志 + 状态；配套**增量编译**（只重编改动文件）与编译产物缓存。
5. **验证方法学支持（按需）** —— SVA 断言（iverilog 支持很弱，需评估「降级为等价 monitor」或引入外部工具）、
   覆盖率、UVM（Icarus 基本不支持 ⇒ 只能靠**可插拔内核**）。**此项优先级取决于用户真实工程是否用 UVM/断言**，
   不建议先做。

### 8.2 结构性大方向（架构级，非独立功能）

- **可插拔仿真内核（Simulation Backend Abstraction）**：把「起进程 / 传参 / 收 log / 收波形 / 终止」抽成接口，
  现役 Icarus 做默认实现，预留 Verilator（快）、外部商用工具（覆盖 UVM/SVA/覆盖率）的接入位。
  这是 8.1-5 与「更大工程」的唯一正路，**建议在本轮之后尽早做接口抽象**（即使只有 Icarus 一个实现）。
- **工程 / 会话模型（Project & Session）**：当前是「一个 exe 实例 = 一个隐式工程」，配置散在 localStorage。
  大方向 = 显式的**工程文件**（顶层、搜索根、include、define、库、波形布局、断点）可保存 / 分享 / 重放。
  Verdi 的 `.rc` / filelist 就是这一类。**这是「专业工具」与「玩具」的分水岭。**
- **脚本化 / 可编程**：命令面板 + 可录制宏（Verdi 有 `.rc` 脚本、TCL）。大方向 = 前端暴露稳定命令 API
  （`__wpsim` 已是雏形），支持脚本批量「加信号 / 设时间 / 截图 / 跑回归」。

### 8.3 明确**不做** / 远期

- 仿真器本身的 HDL 语义实现（不自研内核，只做前端 + 内核编排）。
- FSDB **写出**（Novas 私有格式，无开源授权）；只考虑**读**或转换为 VCD/FST。
- 时序 / STA、功耗分析、门级带 SDF 的完整时序仿真（属另一类工具）。
