...
| 期 | 内容 | 状态 | 关键点 |
|---|---|---|---|
| #86 | 实例 → 模块定义源码跳转 + 源码文件导入 + 例化解析增强 | ⬜ 近期第一项 | 底层：模块定义索引（全量 module 表 `file+moduleLine` 已就绪）需覆盖参数化例化 `#(.p(v))`、命名/位置端口例化、generate 内例化、多文件同名模块歧义；交互（仿 nTrace）：点实例跨文件跳定义行（保留“例化点”次入口），磁盘多选导入 `.sv/.v` 进 `state.files`（= Verdi filelist 语义的本地等价） |
| #76 | 点变量 → 加波形 + 树↔代码双向跳转 + 信号组入 `.wp` | ⬜ 近期第二项（#85 的 VCD 侧已完成） | 底层：**模块体内符号索引**（reg/wire/net/端口/实例名 → 行号 → 映射到 VCD scope 全路径）、多实例歧义候选数据（同名模块 N 次例化 → 让用户选 instance scope）；交互（仿 Verdi Get Signals/nWave）：RTL 树点行加波形、**代码内点变量名加波形**（“中追”式 = #87① 语义，两条最终同一实现）、树↔代码双向跳转、信号组/观察行随 `.wp` 存档恢复 |
...
- **B2 交互：RTL 树信号行 → 画布观察行**：复用 #85 `pickVcdSignalIntoWave` 口径；RTL 行
  无 VCD 数据（未 dump）给中文提示；同名模块多次例化时弹 instance scope 候选选择器。
- **B3 交互：代码 ↔ 树双向跳转**：已有“树 → 代码”；补“代码符号/行 → 反向高亮/定位
  RTL 树与 VCD 树节点”。
- **B4 交互：代码内点变量名 → 加波形**（= #87① / “中追”式）：源码选中/悬停变量 +
  快捷键（仿 Ctrl+W）→ 经 B1 映射 + #85 链路加入波形；不引入单独“信号层次选择框”
  （用户明确不想要），只对代码操作。
...
- **目标布局草案（仿 Verdi nTrace/nWave 三区语义）**：
  ① 层次/信号树区（Get Signals 语义：RTL 树 + VCD 树 + 搜索过滤 + “加入波形”入口）；
  ② 代码区（CM6 + 行内交互：点变量加波形、Active Annotation 位）；
  ③ 波形画布区 + 仿真控制/工具条整理收敛。
