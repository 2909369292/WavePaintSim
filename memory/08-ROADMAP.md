...
## 7. 控件与工具栏规范化（U 线）—— 第二十四轮 2026-09-13 提出
...
### 7.4 目标分层与成员表（推荐方案 D-UI-A）

**L0 菜单栏**（`#menu-bar`，7 个菜单）：不动。

**L1 应用工具带 `#toolbar`（11 项 + `⋯`，从 34 个顶层元素收敛）**
...
| G1 文件 | 打开 / 保存 | |
| G2 编辑 | 撤销 / 重做 / 剪切 / 画笔颜色 | |
| G3 时间 | 步数 / 子步数 | `margin-left:auto` 右对齐，**优先级最高、永不被收** |
| ⋯ | 溢出菜单 | 按优先级收纳 G2 尾部 → G1 尾部等 |

**L2 波形面板上下文带（5 组 / 18 项，接管所有「只对波形有意义」的控件）**

| 组 | 成员 |
|----|------|
| T1 视图 | 缩小 / 放大 / 适应窗口 / 时间游标 |
| T2 绘制 | 画笔 / 橡皮擦 / 选择（框选） |
| T3 状态 | 位状态 / 编辑力度（整步·子步） / 进制（Dec·Hex·Bin） |
| T4 标注 | 箭头 / 时间段 / 标记 / 文本标注 |
| T5 对象 | 添加信号 / 选择对象 / 属性 |
...
**关键取舍**：本方案把真机现有 21 个图标按钮中的 **14 个**（缩放 3 + 绘制 3 + 位状态/力度/进制 3 + 标注 5）**从全局带下沉到波形面板带**。
依据：① 这些动作只对波形窗口有意义（Verdi 的 nWave 也是自带工具条）；② 全局带从 34 个顶层元素 → 14 个，
「乱」的根因消失；③ 面板带是**上下文**带，切到源码面板时自然不显示画笔类控件。
**备选方案 D-UI-B**（改动小、见效少）：全局带保留两行（按组折行）+ 全部加分组间距与溢出菜单 —— 只解决 P-UI-01/02/06/09，
不解决 P-UI-07/08，**不推荐**。二者取一由用户在 U1 拍板（§7.9-①）。
...
| 期 | 内容 | 触发 C1/exe | 验收 |
|----|------|------------|------|
| **U0** | **原型补齐 + 重排**（零风险沙盘）：① 把真机缺的 8 类控件补进 `prototype/`（位状态 / 画笔颜色 / 添加信号下拉 / 编辑力度 / 进制 / 选择对象 / 属性 / 时间偏移类）；② 按 R1~R9 重排 L1/L2/L3，删 `.mk-hintbar`（R9）、收敛 primary（R3）、上下文 chip 移到面板头（R9）、间距刻度化（R5）、面板带 `nowrap + ⋯`（R6）；③ 分段控件 / 下拉做成「点了有反馈」的假交互 | **否**（`prototype/`、`tools/` 不进 `resources.txt`） | `node tools/ui-audit.mjs` → 两档违规 **0**；`node tools/mock-probe.mjs` → **34/34** |
| **U1** | **用户 review 控件终稿**（闸门 **U-1**）：给出每条带的组图 + 控件归位表终稿；同时拍板 §7.9 四条 | 否 | 用户确认（改动方向 / 是否要 D-UI-B） |
...
| **U3** | **真机落地 + 三个真 bug 修复**：R5 间距刻度化（删 5 处散落 margin）→ R6 溢出菜单（修掉 P-UI-02，1440/1280 不再越界）→ R4 尺寸统一（位状态 42→28 网格）→ L2/L3 上下文带拆分（`index.html` DOM 换容器，id 保留） | **是** | `node tools/ui-audit.mjs --real --check` 四档违规 **0**；全量 e2e 绿 |
...
| 3 | **D0 + U2** 面板注册表 + 宿主容器抽象 + 工具带 spec（**像素级零视觉变化**） | U1 | 是 | 1~1.5 天 |
| 4 | **U3** 真机工具带落地 + P-UI-02 真 bug 修复 | U2 | 是 | 1 天 |
...
1. **工具归位**：接受 §7.4 的 **D-UI-A**（14 个波形类控件下沉到波形面板上下文带，全局带 34→14 个元素）？
   还是 **D-UI-B**（全部留在全局带，只做分组 + 刻度 + 溢出）？
   *建议 D-UI-A —— 这是「控制栏乱」的治本方案，也是 Verdi 的做法。*
