...
## D37 · 底部两条状态栏一律删除（用户点名）+ 记忆 / 文档转入「按需精简」模式

**背景（用户原话要点）**：「将最底下的**仿真完成一个输出信号，在代码里双击变量名等**这一行所在的这一栏**删掉**，我不想这一行占用空间」；
「同理，将最底下的 **paint 工作区布局** 等这一文字所在的这一栏**也同样删去**」；
「**更新文档和更新记忆的开销太大了**，本次文档就更新到这里，从本次往后，**除了必要的更新以外，文档只做简单的更新**，没有必要更新的文档也可以不更新了。
**只有遇到非常重要的、觉得必须要记住的东西，才更新到记忆中**，其他的只做简单介绍就可以了」。

**决定**：

1. `#sim-status`（单行状态行）与 `#wp-status-bar` 整条（含 `#wp-status-text` / `#wp-layout` / `#wp-panels` / `#wp-version`）**物理删除**，
   并进入「**已退役，不得复活**」名单；`#sim-console` 子节点**恰好 2 个且顺序固定**：`#sim-console-log` → `#sim-recover`。
   —— **本条取代 D36 第 1 条中「子节点恰好 3 个（log → status → recover）」的旧契约**（旧写法已作废）。
2. `setStatus()` **保留为兼容层**（不删调用点）：文本转发进日志流（`consoleOut`）+ `lastStatusLine` **相邻去重**，不再写任何 DOM 状态位；
   错误类文案按关键字自动走 `error` 配色。版本号自检改由 `consoleOut("[WavePaint] …")` 输出。
3. **记忆 / 文档进入按需精简模式（永久生效）**：默认每轮**只追加** `memory/logs/<date>.md` 一段；
   仅「必须记住」的坑 / 裁决才写 `06-PITFALLS.md` / `07-DECISIONS.md`；**不再每轮全量同步 22 个记忆文件与 INDEX 页脚**。
4. **仍然强制**（不受精简模式影响）：**C1** 改 `js/` `css/` `index.html` / launcher → **必须重建 exe**；**C8** 构建后特征串核验（`rg -a -c "@@LOG:" ≥ 1`）；
   **C2/C3** 代码 commit 与记忆 commit **分开**并 push `main`。

**关联**：`06 P50·P51` / `07 D36（第 1 条子节点契约已作废）` / `08 §7.10 冻结名单` / `memory/logs/2026-09-14.md` / `css/workspace.css §4·§5` / `index.html` / `js/sim/ui-bridge.js` / `js/sim/dock/workspace.js`。

---

## D38 · 顶栏新功能一律走「新菜单 + `data-wpv-*` 自建监听」；帮助菜单只留「关于」；候选编号 #99 起

**背景（第四十八~四十九轮用户要求）**：① 顶栏要有「设置 / 选项」入口，能对**波形框 / 代码框 / 仿真状态框**等增添或编辑，
按专业仿真工具标准增加设置项；②「帮助」删无用内容、**删教程**、「联系」改**极简页**（后续填版权 / 制作人信息）；
③ 按专业仿真软件标准**评估还缺哪些功能**，且只要**实际功能类**（明确排除快捷键这类不改变能力边界的事项）。

**决定**：

1. 设置入口 = **新增一级菜单「窗口」**（`面板与视图设置…` / 面板显隐 / 浮动·停靠 / 布局预设 / 恢复默认布局），
   「视图」菜单追加波形格宽（24/32/48/64/96 + 适应窗口）、代码字号（12/13/15/17）、全屏显示。
2. **接入范式（后续所有顶栏新功能唯一合法方式）**：核心 `handleAction`（C9 一行不能动）不可扩展，
   且 `initMenuHandlers()` 只对**脚本执行时已存在的 `[data-action]` 一次性绑定** ⇒ 新菜单一律用 **`data-wpv-*` 属性 + 自建事件委托**。
   先例 = `js/editor/view-menu.js` + `css/view-menu.css`（`data-wpv-*` 29 项），设置持久化在
   **`localStorage['wavepaint.view.settings.v1']`**（不进 `.wp` 工程档）。
3. 「帮助」菜单**只剩一条「关于 WavePaint」**；`tutorial` / `contact` 两个 action 与 `js/editor/file-menu.js` 中的捕获死码**一并删除、不得复活**；
   「关于」页是极简版（版本 + 构建戳），**版权 / 制作人文案等用户提供后再填**。
4. **候选功能编号 #99 ~ #121 已被占用**（见 `docs/2026-09-14-专业仿真能力评估与功能规划.md` §4），新需求**从 #122 起**编号。
5. 评估口径固定：只收**实际功能类**，不收「快捷键方案 / 动效美化」；§5「明确不做」清单（交互式调试断点单步、覆盖率 / SVA、
   RTL 树显示端口、波形区自带子功能栏、复活 `.helper-box` 一族）**不得作为新增项回炉**。
