...
## D22 · 面板系统重构（dockable / Word 式自由拖拽）的目标形态与四条待裁决项（2026-09-13 第二十二轮，**状态 = 待用户拍板**）

> 与 D20 同性质：**先方案后实施**。本条记录「已经定下来的部分」与「必须由用户拍板的部分」，
> 避免下一位 AI 把方案当成已批准的口径开工。
...
8. **分期闸门**：**#94 D0 = 面板注册表 + 宿主容器抽象（像素级零视觉变化）**，是「回滚成本 = 1 个
   commit」的关键闸门；D0 未验收通过不得进 D1。
...
**待用户拍板（未拍板前不得开工 D1 及以后；D0 也建议等一并拍板）**
...
**关联**：#94（面板系统重构）/ #95（仿真后端能力增强 E1~E5）/ #96（工作区与持久化 W1~W3）见
`03-REQUIREMENTS.md` 表 K；后端缺口 G1~G8 见 08 §6.2；本轮记录见 04 §4.20、日志 2026-09-13。
