...
## H. Verdi 功能规划（2026-09-09 第五轮调研登记；#85 已先行落地）
...
> 交互语义详见 `memory/logs/2026-09-09.md` 第五轮；落地顺序与细节见 `04-PROGRESS.md` §4.5/§4.8。
> 编号口径：原 P1=#76 保留为总纲；#84/#85/#86 是其「细化先行批」，先做先行批，#76 才算推进。

| ID | 需求 | 状态 | 实现思路 / 备注 |
|----|------|------|----------------|
| #83 | Verdi 功能规划总登记（①查看波形 ②添加信号 ③打开 top 自动找例化模块源码 + 其它好用功能） | ⬜ 规划完成，未动代码 | 拆解为 #84~#87 分批落地；建议第一批 = #85 → #86 → #84（见 04 §4.5） |
| #84 | Verdi ①查看波形增强：nWave 缺失能力对齐 | ⬜ | WavePaint 已有 zoom in/out/fit、A/B 游标与 Δ 测量、Bus radix 右键菜单、bit 值菜单、GroupManager、undo/redo。规划补齐：仿真结果 **reload 重载**（重新出图，nWave `L`）、**完整层次路径显示开关**（nWave `h`：信号名显示 scope.path 而非叶名）、**游标快速移到首/末**（nWave `b`/`e`）。只对齐信息层级与导航，不引入 nWave 全量 |
| #85 | Verdi ②添加信号：VCD/结构树里点信号 → 加入 waveform（= #76 P1 第一步，先行） | ✅ 2026-09-09 第十轮完成 | VCD 树点信号→画布「观察行」：`state.vcd` → `buildVcdHierarchy` 完整点分路径 + `toNativeSignal` 注入链路已就绪，ui-bridge.js 新增 `state.simWatches` + `pickVcdSignalIntoWave`（已在画布→提示+滚动定位；不在→登记+push 行；无 VCD/未 dump→中文提示）；观察行 `__simInjected:true` 不进激励、重仿真按路径刷新去重、删行不复活可重加。e2e-rtl 新增 D1~D6 → 15/15。RTL 结构树点行加波形 / 同名多实例歧义选择器仍属 #76 P1 后续。详见 04 §4.8 与日志第十轮 |
| #86 | Verdi ③打开 top → 点例化模块跳其**定义源码文件** + 源码文件导入（filelist 语义） | ⬜ | 现状：RTL 树实例点击只跳「例化点行」（rtl-nav.js 实例节点仅 moduleName/instanceName/line，无定义文件链接）。落地两步：①点实例 → 在 `buildRtlNav` 全量模块表按 `moduleName` 找同名 module → 跨文件切 tab + 跳其 moduleLine；②新增「导入源码文件」入口（磁盘 `.sv/.v` → `state.files` → 自动解析刷新树）。Verdi 语义 = filelist 全录入后模块定义跳转有源码，**并非全盘自动扫盘** |
| #87 | 其它 Verdi 好用功能规划登记 | ⬜ | ①源码行选中变量 → 加波形（nWave Ctrl+W）；②模块全部接口一键入波形（nWave Ctrl+4）；③信号组新建/重命名/批量加入（黄线 + 右键 Add to Waveform）→ 与 #76「信号组入 .wp」合流；④Active Annotation：源码行显示游标时刻信号值（对齐 P2=#77，先登记）。均为交互语义对齐，**不做** nSchema 原理图 / FSDB / 重后端 |
...
- Verdi 规划拆解 #83~#87 已登记（见上表 H）；落地第一批 #85 → #86 → #84——#85 已于 2026-09-09
  第十轮完成（见 04 §4.8），下一步 #86 → #84，完成后主线收 #76 剩余项。
- （用户前几轮提到的远期需求，已明确「目前不着急做」）：复刻“中追”式代码内点变量名→加波形
  （=#87①/#76 语义；VCD 树点信号已由 #85 实现，代码内点变量名仍未做）；侧栏/整体 UI 框架
  重新设计方案；「UI 设计交给其它 AI 是否更好」的咨询。均未排期，等用户主动推进。
