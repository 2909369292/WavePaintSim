...
## H. Verdi 功能规划（2026-09-09 第五轮调研登记，未实现）

> 用户原话要求：「详细研究 Verdi 中以下功能和其他与本项目相关的好用的功能，将其规划到本项目中：
> ① 查看波形；② 添加信号；③ 打开一个 top 时，自动查找所有例化的模块的源码文件。」
> 本轮已完成研究 + 差距对照 + 拆解登记（纯规划，未动代码）。Verdi 真实交互语义详见
> `memory/logs/2026-09-09.md` 第五轮；落地顺序与细节见 `04-PROGRESS.md` §4.5。
...
| #85 | Verdi ②添加信号：VCD/结构树里点信号 → 加入 waveform（= #76 P1 第一步，先行） | ⬜ | 数据已就绪：`state.vcd` → `buildVcdHierarchy` 信号带完整点分路径（vcd-index.js）；注入链路 `toNativeSignal`（ui-bridge.js L247/L300/L797）。现 `renderVcdTree` 的 `onSignalPick` 只 `setStatus`（ui-bridge.js L424），需接「加入画布」。要处理：未 dump 信号中文提示（不静默）、同名多实例歧义选择器、加入后滚动到可见区 |
