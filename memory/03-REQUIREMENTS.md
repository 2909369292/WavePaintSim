...
> **2026-09-10 第十五~十九轮**：近期主线第二项 **#76** 已**全部落地** —— **B1（符号索引数据层，
> 第十五轮）**、**B4（代码内点/选中变量 → 加波形 = 唯一加信号主路径，第十六轮）**、
> **B3（代码 ↔ 树双向跳转 = 代码光标反向高亮 RTL/VCD 树，仿 nTrace「光标即高亮」，第十七轮）**、
> **B5（画布观察行随 `.wp` 工程存档与恢复，第十八轮）**；第十九轮再落地 **#87②（模块/实例全部接口
> 一键入波形，仿 nWave `Ctrl+4`；实际触发键 = `Ctrl+Alt+4`）** —— **#76 + #87①②③ 全部收口**，
> **下一项 = 侧栏 / 整体 UI 重构设计方案（08 §2，先出方案给用户 review，不直接开工）**。
> 详见 04 §4.13/§4.14/§4.15/§4.16/§4.17、06 P32/P33/P34/P35/P36、07 D16/D17/D18/D19、08 §1.2/§1.3。
...
| #86 | Verdi ③打开 top → 点例化模块跳其**定义源码文件** + 源码文件导入（filelist 语义） | ✅ 全部完成（**A1~A4** 2026-09-10 第十四轮；子项「服务在线性」2026-09-10 第十三轮） | 现状：RTL 树实例点击只跳「例化点行」（rtl-nav.js 实例节点仅 moduleName/instanceName/line，无定义文件链接）。落地两步：①点实例 → 在 `buildRtlNav` 全量模块表按 `moduleName` 找同名 module → 跨文件切 tab + 跳其 moduleLine；②新增「导入源码文件」入口（磁盘 `.sv/.v` → `state.files` → 自动解析刷新树）。Verdi 语义 = filelist 全录入后模块定义跳转有源码，**并非全盘自动扫盘**。**第十一轮确认近期先做 #86→#76**；落地拆解 A1~A4 见 08 §1.1。**2026-09-10 第十三轮**：用户当轮诉求是「每次点仿真必须真正调用本地仿真服务并成功」，已把 **#86 的独立子项「服务在线性根治」** 做掉（固定端口 17817 + 身份标识判活 + `ivlRoot` 换版本首启崩溃修复 + 页面侧自动重连/重拉/自动重试），见 04 §4.11。**2026-09-10 第十四轮**：**A1~A4 全部实施完成** —— A1 源码级实例扫描器 `scanInstances` + 模块定义索引 `collectModuleDefs`/`resolveModuleDef`（`rtl-nav.js`）；A2 实例行**左键跳模块定义 / 右键(Alt+左键)跳例化点**、黑盒中文提示；A3 侧栏「导入源码」磁盘多选入口（同名去重）；A4 源码集合随 `.wp` 存档/恢复（包裹核心 `buildDocumentJson`/`loadFromFileContent`，旧工程向后兼容）。见 04 §4.12、06 P31。**下一步整线进入 #76 B1** | 
| #87 | 其它 Verdi 好用功能规划登记 | ✅ ①②③ 全部落地 2026-09-10 | ①源码行选中变量 → 加波形（nWave Ctrl+W）——**已并入 #76 B4（代码内点变量加波形）落地**（第十六轮）；②模块全部接口一键入波形（nWave Ctrl+4）——**第十九轮落地**（实际触发键 `Ctrl+Alt+4`，「全部接口」只指模块端口，体内 `wire/reg` 仍走 B4 单点加；见 04 §4.17 / 07 D19 / 06 P36）；③信号组新建/重命名/批量加入（黄线 + 右键 Add to Waveform）→ 与 #76「信号组入 .wp」合流（B5）；④Active Annotation：源码行显示游标时刻信号值（对齐 P2=#77）——**随 #77 打入远期**。均为交互语义对齐，**不做** nSchema 原理图 / FSDB / 重后端。**2026-09-09 第十一轮用户将 #87 整体优先级提高** |
...
- Verdi 规划拆解 #83~#87 已登记（见上表 H）；**第十一轮重排后的近期主线 = #86 → #76 →
  ...
  04 §4.5/§4.9、08 §1）。#85 已于第十轮完成（见 04 §4.8）。**截至第十九轮，主线 #86（A1~A4）+
  #76（B1~B5）+ #87①②③ 已全部落地**；**下一项 = 侧栏 / 整体 UI 重构（08 §2.1~§2.9，
  第二十轮方案已成文，状态 = 待用户 review，拍板后按 08 §2.6 开工 P0，拍板前不直接开工）**。
- **#84 波形查看增强、#77 Active Annotation**：2026-09-09 第十一轮用户**推迟打入远期**，
  不再排期（见 08 §3）。
- **侧栏 / 整体 UI 重构设计**：优先级已提高（第十一轮），作为与主线并行的方案线推进。
  **2026-09-10 第二十轮方案已成文**（08 §2.1~§2.9：现状问题清单 + 仿 Verdi 三区布局草案 +
  面板按钮归位表 + 主线衔接点 + 契约面冻结清单 + P0/P1/P2 分期 + 「UI 设计交给其它 AI」评估 +
  交互清单），**状态 = 待用户 review**，拍板后按 08 §2.6 开工 **P0（纯 CSS/布局）**；
  方案摘要见 04 §4.18、决策 07 D20。「UI 设计交给其它 AI」评估结论 = **可行但只限出稿**
  （布局对比图 / 视觉规范 / 交互流程稿），**契约面判定与 e2e 影响面须由本仓把关**。
...
- 复刻“中追”式代码内点变量名 → 加波形（=#87①/#76 语义）：VCD 树点信号已由 #85 实现；
  ...
  **模块/实例全部接口一键入波形（#87②）已于第十九轮落地**（触发键 `Ctrl+Alt+4`，仿 nWave `Ctrl+4`；
  「全部接口」只指模块端口，体内 `wire/reg` 仍走 B4 单点加 —— 见 04 §4.17 / 07 D19 / 06 P36）。
