...
## K. 面板系统重构 + 仿真后端增强（2026-09-13 第二十二轮登记，**纯规划；待用户拍板后开工**）
...
> 安排。」→ 本轮完成**后端能力盘点 + 面板形态盘点 + 差距分析 + 选型 + 分期**，落盘在
> `08-ROADMAP.md` **§6**（§6.1 后端能力表 / §6.2 后端缺口 G1~G8 / §6.3 前端现状 / §6.4 差距 /
> §6.5 选型 / §6.6 目标架构 / §6.7 契约面与 e2e 影响面 / §6.8 分期 / §6.9 验收与回滚 /
> §6.10 四条待拍板 / §6.11 与远期关系）。**本轮零产品代码改动，不触发 C1。**

| ID | 需求 | 状态 | 实现思路 / 备注 |
|----|------|------|----------------|
| #94 | **面板系统重构：dockable 多窗口 / Word 式自由拖拽**（波形 / 代码 / 层次 / 控制台四类面板可停靠、拖拽重排、切分、Tab、浮出、最大化、布局持久化） | ⬜ 方案已出，**待用户拍板 §6.10 四条** | 分 5 期：**D0** 面板注册表 + 宿主容器抽象（**零视觉变化**，id 契约天然不变）→ **D1** 停靠引擎（左/中/右/下四区 + 拖拽换区/排序/切分 + Tab 组；画布进 CENTER、源码进 RIGHT、RTL/VCD 进 LEFT、TB/状态进 BOTTOM）→ **D2** 浮动窗口（**页内浮动层**，非 OS 窗口）+ 最大化 + 键盘 → **D3** 布局持久化（sessionStorage 保底 + localStorage 跨会话 + 3 套预设 + reset）→ **D4** 打磨（手势/空态/a11y/拖拽期暂停画布重绘）。**选型 = 自研轻量引擎（≈800~1200 行，新增 `js/sim/dock/*.js`）为主**，Lumino/golden-layout 为备选（D1 后若手感不达标再评估）。**D0 是关键闸门**：视觉零变化、回滚成本 = 1 个 commit。**08 §2.6 的三项剩余（VCD 树移入波形区 / TB 控制带收敛 / 层次树左置）直接并入 D1**，不再单独排期 |
| #95 | **仿真后端能力增强：从「画布即激励」扩到「通用仿真」** | ⬜ 方案已出，未开工（与 #94 正交，可并行） | 缺口清单见 08 §6.2：**G1 TB 只读（最大缺口）**→ E1 手写 TB 双态（`#tb-source` 去 readonly + 「自动生成/手动编辑」+ 覆盖前确认 + 随 `.wp` 存档）；**G2 无日志/进度** → E2 成功路径也回传 `stdout/stderr`（`@@LOG:` 分段，**旧格式兼容**）+ 控制台面板 + 请求可取消；**G3 编译选项不可配** → E3 `-D`/`+define+`/`-I`/`-y`/filelist 导入 + 随档；**G5 大 VCD** → E4 分片/流式 + 解析不卡 UI（可选）；**G7 单顶层** → E5 多顶层/多 TB（可选）。**E1+E2 = 用户「通用仿真器」诉求的最小充分集**，建议优先 |
| #96 | **工作区与持久化配套** | ⬜ 方案已出，未开工 | W1 布局随会话（= #94 D3）；**W2 工程自动存档 / 崩溃恢复**——复用**已实现但前端未接线**的 `/api/snapshot`（`%TEMP%` 保留 20 份 + `latest.json`），重开页面**提示**用户恢复（**恢复必须用户点选，绝不自动覆盖**，遵守「无自动保存」红线）；W3 布局写进 `.wp`（**待 §6.10 第 4 条拍板**，默认不做） |
