...
|---|---|
| 主线 | `main` |
| 当前版本 | 真机 `v0.4.0 build 2026-09-13 16:52:06 83fab85`（`83fab85` = **构建时 HEAD**，承载第二十五轮代码的 commit 是它的下一个，勿误判落后）；原型 `v0.4.0-mock build 2026-09-13 17:08:12 2d1636a` |
| 最近完成 | **第二十六轮（2026-09-13）= 原型打包为可双击运行的 `WavePaintMockup.exe`（独立启动器 `MockupLauncher.cs` + 独立构建 `build-prototype.ps1` + 无浏览器冒烟 `tools/mockup-exe-smoke.mjs` 41/41）**：用户原话「提供的**网页链接无法打开**，请按照**原本之前的方式**一样打包成一个**可直接运行的点 exe** 供我 review」→ 把第二十五轮 U0-R 原型从「起 dev-server + 手输 URL」改为**双击即用、零依赖、零命令行**。**未触发 C1 / 真机 exe 一行未动**。详见 04 §4.24 / 本文件 §3.1 |
| 最近完成文档 | **第二十六轮记忆同步**（2026-09-13）：`04-PROGRESS.md` **新增 §4.24**（第二十六轮全记录：用户请求与判定 / 交付物清单表 / 与真机三处身份分离表 / 实测表 / 红线）+ §1 快照切第二十六轮 + §2 时间线新增行；`09-HANDOFF.md` §1 快照 + **新增 §3.1（原 3.1~3.7 顺延为 3.2~3.8）** + §6 交付提醒 + §9 本轮遗留；`07-DECISIONS.md` 新增 **D25**；`02-WORKFLOW.md` 加原型 exe 构建/冒烟命令与 C1 例外说明；`06-PITFALLS.md` 新增 **P38**；`05-LOGS.md` 索引 + 当日日志 `logs/2026-09-13.md`；`INDEX.md` 页脚。**叠加生效**的仍是第二十六轮交付方式：原型 review = 双击 `WavePaintMockup.exe` |
| 测试基线 | regression **79/79**；e2e-rtl **61/61**（#85 D1~D6 + 第十二轮 B3 + 第十四轮 E1~E6 + 第十五轮 F1~F9 + 第十六轮 G1~G7 + 第十七轮 H1/H2/H3/H4/H4b/H5/H6 + 第十八轮 I1~I6 + 第十九轮 J0~J7）；**e2e-ui 88/88**（真实 Edge；I0~I8 + 第二十五轮 I1 改口径 + 新增 I4c 指针拖拽）；e2e-sim 0 失败；probe-param 全过；真 exe 冒烟通过；**原型侧（第二十六轮新增）= `node tools/mockup-exe-smoke.mjs` 41/41 + `node tools/mock-probe.mjs` 34/34（dev-server 与 exe 服务双跑）+ `ui-audit --mock` 两档违规 0** |
| 当前阻塞 | 无（U1 闸门已拍板 → **U0-R 顶栏统一**；`08 §7.9-③④` 用户未提异议，按默认执行）。当前待办 = **等用户 review 第二十六轮交付的 `WavePaintMockup.exe`**（双击即用的 U0-R 原型假界面） |
| 下一步 | **① 等用户 review `WavePaintMockup.exe`**（双击 `D:\Files\Code\波形\WavePaintMockup.exe` → Edge 无地址栏窗口打开原型，顶部绿条显示构建戳；关窗约 12s 后进程自动退出；**零依赖**）。**② 未提异议即按 `08 §7.8` 总排期推进**（U 线与 D 线合并；**D-UI-A 已作废**：真机 21 个图标按钮**不下沉**、全留 `#toolbar`，U3 只做分组 / 刻度 / 尺寸 / `⋯` 溢出）：**U2 工具带 spec 数据驱动化**（`js/sim/ui/toolbar-spec.js`，24 个冻结 id 不改名；触发 C1 重建 exe）与 **D0 面板注册表 + 宿主容器抽象**（像素级零视觉变化）**合并同批** → **U3 真机落地 + 修 P-UI-02**（真机 1440 溢 32px / 1280 溢 192px）→ **D1 停靠引擎** → D2 页内浮动 → D3 持久化（不写 `.wp`）→ D4+U4。**#95（G1~G8）用户明确暂缓**；**#84 / #77 / #78 远期**；RTL 树只做代码层级浏览；服务自愈（C19 不变量）为独立专项。**每批次都要 C1 重建 exe + C8 核验 + 全量测试 + C17 记忆同步** |
...
### 3.1 第二十六轮（当前生效，2026-09-13）：**原型打包为可双击运行的 exe（交付方式升级：网页链接 → 零依赖 exe）**
...
### 3.2 第二十五轮（上一轮，2026-09-13）：**U0-R 顶栏统一（撤销 D-UI-A）+ 真机 splitter 高度模型根本重写 + 三条真 bug + 混淆残留清零**
...
### 3.3 第二十四轮（更早，2026-09-13）：**控件与工具栏规范化 U 线 —— 方案定稿（08 §7）+ 审计工具转正 + 原型 U0 重排完成（其 U1 闸门已由第二十五轮用户裁决 U0-R 关闭）**
...
### 3.4 第二十三轮（更早，2026-09-13）：**用户四条拍板 + 零后端纯前端 UI 原型已交付，等用户 review（D-1 闸门）**
...
### 3.5 第二十二轮（更早）：**面板系统重构（dockable / Word 式自由拖拽）方案已出（第二十二轮，纯规划）**
