...
| 最近完成 | **第二十二轮（2026-09-13）= 面板系统重构方案 + 后端能力盘点（纯规划，零产品代码改动）**：终局目标 = 模仿 Verdi 的**波形窗口 / 代码窗口 / 代码树窗口**等，像 **Word 一样自由排列组合与自由拖拽**（UI 要大改）→ 交付 **`08-ROADMAP.md` §6**（§6.1 后端能力表 / §6.2 后端缺口 **G1~G8**（**G1 TB 只读 = 最大缺口**、G2 无日志与进度、G3 编译选项不可配、G4 无取消、G5 VCD 全量文本、G6 `/api/snapshot` 未接线、G7 单顶层、G8 解析器为 SV 子集）/ §6.3 前端现状 / §6.4 差距表 / §6.5 选型（**自研 dock 引擎为主**，Lumino / golden-layout 备选，React 系与 iframe 多窗否决）/ §6.6 目标架构（`PanelRegistry` + `LayoutTree` + `DockDnD` 三层；四区 + 页内浮动层）/ §6.7 契约面与 e2e 影响面 / §6.8 分期 **#94 D0~D4 + #95 E1~E5 + #96 W1~W3** / §6.9 验收与回滚 / §6.10 **四条待拍板** / §6.11 与远期关系）。**本轮未重建 exe**（C1 未触发）。此前最近一次代码交付 = **第二十一轮（2026-09-11）侧栏「可拖拽多面板」**（4 卡折叠 + 纵向 splitter + 侧栏拖宽 + sessionStorage；**24 个冻结 id 一个未改**；顺带修掉矮视口 `#sim-run` 被裁致「点仿真无响应」的真 bug）；再往前 = #87②（第十九轮）、#76 B1~B5、#86 A1~A4、第十三轮服务自愈、#85、RTL 树瘦身、第六轮 5 项 + #93 收官 |
| 最近完成文档 | **第二十二轮记忆同步**（2026-09-13）：**`08-ROADMAP.md` 新增 §6**（面板系统重构方案，约 200 行，含 §6.1~§6.11）、**`03-REQUIREMENTS.md` 新增表 K**（#94 / #95 / #96）、`04-PROGRESS.md` 新增 **§4.20** + §1 快照（日期改 2026-09-13 + 新增「最新一轮」行 + 重写「下一步主线」行）+ §2 时间线补第二十二轮、**`07-DECISIONS.md` 新增 D22**（dockable 目标形态 + **四条待拍板**）、本文件 §1 三行 + §3 切第二十二轮、`05-LOGS.md` 索引、`memory/logs/2026-09-13.md`。叠加生效的仍是：**加信号主路径 = 代码内点/选中变量**（#76 B4）；**RTL 树只做代码层级浏览**；**服务在线性不变量**（C19 / D15）；**契约面冻结**（D20 / 08 §2.5，新增 `#sim-status` 常驻 + `#sim-run` 可点两条） |
...
| 当前阻塞 | 无 |
| 下一步 | **唯一下一步 = 等用户对 `08 §6.10` 四条待拍板项裁决**（① 是否解除「侧栏保持右侧」；② 是否解除「层次树暂不左置」；③ 浮出 = **页内浮动**（推荐）vs 真独立窗口；④ 布局是否写进 `.wp`（默认不写）；AI 建议 ①②解除 / ③页内 / ④不写）。**拍板后按 `08 §6.8` 开工**：**#94 D0（面板注册表 + 宿主容器抽象，像素级零视觉变化 = 关键闸门）→ D1（四区停靠 + 拖拽换区/排序/切分 + Tab）→ D2（页内浮动 + 最大化 + 键盘）→ D3（sessionStorage + localStorage + 3 预设 + reset）→ D4（打磨）**；**#95 E1（TB 可编辑）+ E2（日志 / 进度 / 取消）与 #94 正交、建议并行优先**（用户「通用仿真器」诉求的最小充分集）。**08 §2.6 的三项剩余（VCD 树移入波形区 / TB 控制带收敛 / 层次树左置）已并入 #94 D1**。**#84 / #77 / #78 仍为远期**（08 §3）。开工前先读 **08 §6、07 D22、04 §4.20、06 P32~P36**；服务在线性不变量见 02 C19 / 07 D15 |
...
## 3. 当前进度：**面板系统重构（dockable / Word 式自由拖拽）方案已出（第二十二轮，纯规划）**；下一项 = **等用户对 08 §6.10 四条拍板后开工 #94 D0**

> **2026-09-13 第二十二轮（当前生效，纯规划 / 零产品代码改动 / 不重建 exe）**：用户要求「确认现在
