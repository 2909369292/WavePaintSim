...
| 最近完成 | **第二十四轮（2026-09-13）= 控件与工具栏规范化 U 线：方案定稿（`08 §7`）+ 审计工具转正 + 原型 U0 重排完成（等用户 U1 拍板）**：用户 review 第二十三轮原型的结论 = **形态层通过**（原话「UI 排布在大体上基本正确」）但**控件层要改**（「小的按钮上的排布以及控制栏的排布还需要斟酌，现在似乎有一些混乱」），并要求「给出之后的工作方案」→ 本轮**只做控件层**（**U0~U4**，形态层归 §6 D 线不动）。① **`08 §7` 全文**：§7.2 实测问题清单 **P-UI-01~09**（含 **P-UI-02 = 真机 `#toolbar` 1440 溢 32px / 1280 溢 192px 的真 bug**）、§7.3 铁律 **R1~R9**、§7.4 分层 **L0~L4** + 主方案 **D-UI-A**（14 个波形类控件从全局带下沉到波形面板带，全局带 **34→14** 个顶层元素）/ 备选 D-UI-B、§7.5 刻度变量、§7.6 `⋯` 溢出降级、§7.7 U0~U4、§7.8 **总排期 ≈8~9 工作日**、§7.9 四条待拍板；② **`tools/ui-audit.mjs` 转正**（真实 Edge headless + CDP 量测，`--mock`/`--real`/`--all`/`--check`/`--widths=`，`violationsOf()` 与 R3/R5/R6/R8/R9 一一对应）；③ **原型 U0 重排**（`prototype/ui-mockup.{html,css,js}`：`#toolbar` → L1 11 项 + `⋯`、面板带 `.mk-tbg` 分组 + `⋯`、`:root` 6 个刻度变量、删 `.mk-hintbar`、状态栏三段化 + `#st-ctx`、新增 `setupBands/Flyouts/CtlFeedback`）。**实测全绿**：`tools/mock-probe.mjs` **34/34**；`tools/ui-audit.mjs` 原型 mock **1680/1280 两档违规 0**（此前 3/6）；**真机基线未修 = 1440 溢 32px / 1280 溢 192px（U3 修）**。**C1 未触发、未重建 exe**（`prototype/**` 与 `tools/**` 均不进 `resources.txt`） |
| 最近完成文档 | **第二十四轮记忆同步**（2026-09-13）：`04-PROGRESS.md` **新增 §4.22**（第二十四轮全记录：三件交付物 / P-UI-01~09 / R1~R9 / L0~L4 与 D-UI-A / 原型 U0 改动清单 / 实测结果 / 总排期 / 不做的事）+ §1 快照（**新增「原型 / 控件测试基线」行** + 第二十四轮行置前 + 第二十三轮行改标「上一轮」）+ §2 时间线补第二十四轮；`07-DECISIONS.md` **新增 D23**（控件与工具栏规范化十条口径 + D-UI-B 对比 + U-1 闸门 + 「为什么先立规再改真机」）；`08-ROADMAP.md` **§7 全文**（约 130 行，本轮主要产物）；`02-WORKFLOW.md` §3 验证门槛表**新增 `tools/mock-probe.mjs` 与 `tools/ui-audit.mjs --all --check` 两行**并校正 regression / e2e-ui / e2e-rtl 的条目数与当前基线一致；`03-REQUIREMENTS.md` 表 K 新增 **#97**（🟡 待 U1 拍板）；本文件 §1 三行 + §3 切第二十四轮（原 3.1/3.2 顺延为 3.2/3.3，并把错位的「目标 / 建议顺序 / 不要做」改号 3.4/3.5/3.6）；`05-LOGS.md` 索引行；`memory/logs/2026-09-13.md` 追加第二十四轮段；`INDEX.md` 页脚 + 上一轮折叠块。叠加生效的仍是：**加信号主路径 = 代码内点/选中变量**（#76 B4，**不是** RTL 树）；**RTL 树只做代码层级浏览**；**契约面冻结**（D20 / 08 §2.5，含 `#sim-status` 常驻可见 + `#sim-run` 始终可点）；**服务在线性不变量**（C19 / D15）；**先方案后实施**（D20） |
...
| 当前阻塞 | 无 |
| 下一步 | **唯一下一步 = 等用户在 U1 对 `08 §7.9` 四条拍板**（① 是否走主方案 **D-UI-A**（14 个波形类控件下沉、全局带 34→14）还是备选 D-UI-B；② `#sim-run` 是否保留在全局带常驻；③ 窄窗口用 `⋯` 溢出菜单还是允许折行；④ 刻度值 28·24·4·12）。看实物的两条路：`node tools/dev-server.mjs 8951` → **`http://127.0.0.1:8951/prototype/ui-mockup.html`**（**必须 http**），或直接看截图 `.e2e-tmp/ctl-1680-toolbar.png` / `ctl-1280-ovf-open.png` / `ctl-full-1680.png`。**拍板后按 `08 §7.8` 总排期推进**：**D0 + U2**（面板注册表 + 宿主容器抽象 + 工具带 `js/sim/ui/toolbar-spec.js` 数据驱动化，24 个冻结 id 不改名；**像素级零视觉变化**是 D0 闸门，回滚 = 1 commit）→ **U3**（真机工具带落地 + **修 P-UI-02 真 bug**：1440/1280 不再越界）→ **D1**（停靠引擎：把原型的 `split`/`tabs`/`hitTest`/落点框搬进 `js/sim/dock/*.js`；`08 §2.6` 三项剩余 —— VCD 树移入波形区 / TB 控制带收敛 / 层次树左置 —— 直接并入本期，不单独排期）→ **D2**（页内浮动 + 最大化 + 键盘）→ **D3**（持久化 session + 跨会话 + 3 预设 + reset，**不写 `.wp`**）→ **D4 + U4**（打磨 + `ui-audit --check` 进提交前清单）。**每批独立 commit + push（C2/C3）；D0/U2/U3/D1/D2/D3 各触发一次 C1 重建 exe + C8 核验 + 全量 e2e + C17 记忆同步**。**#95（G1~G8）仍为用户明确暂缓**（方案保留在 08 §6.2，**勿删**，恢复前须重新确认意图）；**#84 / #77 / #78 仍为远期**（08 §3）；**#88~#92 已于第九轮全部 ✅，勿重做**；开工前先读 **08 §7 + 07 D23 + 04 §4.22** |
...
### 3.1 第二十四轮（当前生效，2026-09-13）：**控件与工具栏规范化 U 线 —— 方案定稿（08 §7）+ 审计工具转正 + 原型 U0 重排完成，等用户在 U1 拍板**
...
### 3.2 第二十三轮（上一轮）：**用户四条拍板 + 零后端纯前端 UI 原型已交付，等用户 review（D-1 闸门）**
...
### 3.3 第二十二轮（更早）：**面板系统重构（dockable / Word 式自由拖拽）方案已出（第二十二轮，纯规划）**
