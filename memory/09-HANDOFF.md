...
|---|---|
| 主线 | `main` |
| 当前版本 | `v0.4.0 build 2026-09-13 16:52:06 83fab85`（`83fab85` = **构建时 HEAD**，承载第二十五轮代码的 commit 是它的下一个，勿误判落后） |
| 最近完成 | **第二十五轮（2026-09-13）= U0-R 顶栏统一（撤销 D-UI-A）+ 真机面板 splitter 高度模型根本重写 + 三条真 bug 修复 + 混淆残留清零**：用户四条原话 ①「**波形的时间标 1234 等错误地放在了源码区**」②「**编辑栏先不做修改**……保存/撤回/放大缩小图标**还放到统一顶栏**，**波形区的子波形区现在不添加功能栏**」③「各个窗口的**大小拖拽有 bug，和鼠标坐标对不上**，而且检查显示等其他 bug」④「**已有绘图 UI 尽量不做改动**，直接照搬即可；如果是很混淆过的，请查明之前的解混淆与剔除工作」。**① 原型 U0-R**：14 个下沉控件全部收回 `#toolbar`（5 组 18 项 + `data-ovp`），**`panelWave()` 整条 `.mk-toolrow` 删除 = 波形区零子功能栏**，`mk-version` → `mock-3（顶栏统一 U0-R）`；决策 **07 D24** / **08 §7.4·§7.9①·§7.10**。**② 真机 `js/sim/panel-layout.js` splitter 重写**（「拖拽和鼠标坐标对不上」真根因）：新增 `cardBaseline`/`pairSlackPx`/`resizePairTo`/`resizePair`，`applyWeights` = `flexBasis 基线px` + `flexGrow 占比*100` + `flexShrink 0`，**`applyMinHeights` 同源同时写 `minHeight` 与 `flexBasis`**。**③ 三条真 bug**：a) **时间标跑进源码区**根因 = 原型 `.mk-gutter` 缺 `white-space:pre`（`gutW 369→32`、`bodyW 59→396`，修复后 `ticksInSource 0`）；b) 原型 `.mk-tick` 类名冲突 → 波形刻度改 `.mk-wave-tick`；c) 原型信号名列宽拖拽失效（`closest`→`querySelector`）+ 时间轴与波形轨横滚同步。**④ 混淆复查**：`js/wavepaint.clean.js` **无残留**（`_0x`/`\xNN` 均在注释或为产品功能），另清 **9 个残留 `_0x_` 前缀局部变量**（`_0x_wpf*` → `wpf*`，32 处）→ 正文 `_0x` 归零。**实测（全部实跑）**：e2e-ui **88/88**（I1 改口径 + 新增 I4c 指针拖拽）、regression **79/79**、e2e-rtl **61/61**、e2e-sim 0 失败、mock-probe **34/34**、ui-audit --mock 1680/1280 违规 **0**；真机四档数字不变（**U3 修 P-UI-02**）。**C1 触发 → exe 已重建**（22,015,488 B / 2026-09-13 16:52:06 / `v0.4.0 build 2026-09-13 16:52:06 83fab85`，C8 特征串核验通过、`_0x_wpf`=0） |
| 最近完成文档 | **第二十五轮记忆同步**（2026-09-13）：`04-PROGRESS.md` **新增 §4.23**（第二十五轮全记录：用户四条原话对账表 / 原型 U0-R / 真机 splitter 新模型 / 真 bug 清单表 a~c / 实测表 / e2e 三条断言改动 / exe 与 C8 / 红线）+ §1 快照（**新增「最新一轮（第二十五轮）」行** + `当前阻塞`/`下一步主线`/`测试基线`/`原型控件测试基线`/`UI 基线`/`交付提醒` 全部更新，e2e-ui 86→88）+ §2 时间线补第二十五轮；`07-DECISIONS.md` D23 标题加「⚠ 第 8 条被 D24 推翻」+ **新增 D24**（U0-R 回退决策 + 5 条用户裁决表 + splitter 基线模型技术口径 + e2e 断言口径 + 混淆复查结论）；`08-ROADMAP.md` §7 标题加「第二十五轮修订（U0-R）」警示 + §7.4 标「⚠ D-UI-A 作废 → U0-R」+ 成员表重排为 G0~G8 + `⋯`（14 个下沉控件收回）+ §7.7 U0/U1/U3 行同步 + §7.8 总排期 + §7.9① 关闭、③④ 标「按默认执行」+ §7.10 新增两条红线；`03-REQUIREMENTS.md` **#97 行措辞**改「U1 已由用户裁决 = 顶栏统一（U0-R），D-UI-A 作废」；`05-LOGS.md` 索引补第二十五轮行；`memory/logs/2026-09-13.md` 追加第二十五轮段；本文件 §1 五行 + **§3 新增 3.1（原 3.1~3.6 顺延为 3.2~3.7）**；`INDEX.md` 页脚 + 上一轮折叠块。叠加生效的仍是：**加信号主路径 = 代码内点/选中变量**（#76 B4，**不是** RTL 树）；**RTL 树只做代码层级浏览**；**契约面冻结**（D20 / 08 §2.5，`#sim-status` 常驻可见 + `#sim-run` 始终可点）；**服务在线性不变量**（C19 / D15）；**先方案后实施**（D20）；**波形区不加子功能栏 + 已有绘图 UI 不照改**（D24 / 08 §7.10） |
| 测试基线 | regression **79/79**；e2e-rtl **61/61**（#85 D1~D6 + 第十二轮 B3 + 第十四轮 E1~E6 + 第十五轮 F1~F9 + 第十六轮 G1~G7 + 第十七轮 H1/H2/H3/H4/H4b/H5/H6 + 第十八轮 I1~I6 + 第十九轮 J0~J7）；**e2e-ui 88/88**（真实 Edge；第二十一轮 I0~I8 + **第二十五轮 I1 改口径**〔`flex-basis` = 卡片基线 px + `flexGrow` 占比 + `flexShrink 0`〕**+ 新增 I4c 指针拖拽**〔拖 DY → 上下卡各 ±DY 且总高守恒、同手势拖回可逆〕）；e2e-sim 0 失败（含「仿真后不自动灌信号」）；probe-param / probe-tutorial / probe-addbtn / probe-simfail 全过；**`tools/mock-probe.mjs` 34/34**（原型渲染 + 交互 + 布局体检 F1~F10）；**`tools/ui-audit.mjs --mock` 1680/1280 两档违规 0**；真机 `ui-audit --real` 未修 = 1920 违规 1 / 1440 违规 3（溢 32px）/ 1280 违规 3（溢 192px）= **U3 范围**；#82 真机冒烟通过；#93 绑定一致性 BINDINGS OK；`verify-recovery2.mjs` 17/17 + `probe-protocol.mjs` |
| 当前阻塞 | 无（**U1 闸门已实质拍板**：用户第二十五轮直接裁决「波形区不再有子功能栏、功能栏统一放顶栏」→ **否决 D-UI-A、采纳顶栏统一（U0-R）**；`08 §7.9-③④`（`⋯` vs 折行 / 刻度 28·24·4·12）用户未提异议，按默认执行。当前待办 = **等用户 review 第二十五轮原型**，然后进 D0+U2） |
| 下一步 | **① 等用户 review 第二十五轮原型（U0-R 顶栏统一）**：`node tools/dev-server.mjs 8951` → **`http://127.0.0.1:8951/prototype/ui-mockup.html`**（**必须 http**）。**② 未提异议即按 `08 §7.8` 总排期推进**（U 线与 D 线合并；**D-UI-A 已作废**：真机 21 个图标按钮**不下沉**、全部留在 `#toolbar`，U3 只做分组 / 刻度 / 尺寸 / `⋯` 溢出）：**D0 + U2**（面板注册表 + 宿主容器抽象 + 工具带 `js/sim/ui/toolbar-spec.js` 数据驱动化，24 个冻结 id 不改名；**像素级零视觉变化**是 D0 闸门，回滚 = 1 commit）→ **U3**（真机工具带落地 + **修 P-UI-02 真 bug**：1440/1280 不再越界）→ **D1**（停靠引擎：把原型的 `split`/`tabs`/`hitTest`/落点框搬进 `js/sim/dock/*.js`；`08 §2.6` 三项剩余 —— VCD 树移入波形区 / TB 控制带收敛 / 层次树左置 —— 直接并入本期）→ **D2**（页内浮动 + 最大化 + 键盘）→ **D3**（持久化 session + 跨会话 + 3 预设 + reset，**不写 `.wp`**）→ **D4 + U4**（打磨 + `ui-audit --check` 进提交前清单）。**每批独立 commit + push（C2/C3）；D0/U2/U3/D1/D2/D3 各触发一次 C1 重建 exe + C8 核验 + 全量 e2e + C17 记忆同步**。**#95（G1~G8）仍为用户明确暂缓**（方案保留在 08 §6.2，**勿删**，恢复前须重新确认意图）；**#84 / #77 / #78 仍为远期**（08 §3）；**#88~#92 已于第九轮全部 ✅，勿重做**；开工前先读 **08 §7 + 07 D24 + 04 §4.23** |
...
### 3.1 第二十五轮（当前生效，2026-09-13）：**U0-R 顶栏统一（撤销 D-UI-A）+ 真机 splitter 高度模型根本重写 + 三条真 bug + 混淆残留清零**
...
### 3.2 第二十四轮（上一轮，2026-09-13）：**控件与工具栏规范化 U 线 —— 方案定稿（08 §7）+ 审计工具转正 + 原型 U0 重排完成（其 U1 闸门已由第二十五轮用户裁决 U0-R 关闭）**
...
### 3.3 第二十三轮（更早）：**用户四条拍板 + 零后端纯前端 UI 原型已交付，等用户 review（D-1 闸门）**
...
### 3.4 第二十二轮（更早）：**面板系统重构（dockable / Word 式自由拖拽）方案已出（第二十二轮，纯规划）**
...
- 2026-09-13 第二十五轮完成 **U0-R 顶栏统一 + 真机 splitter 高度模型重写 + 三条真 bug + 混淆残留清零**
  ...
  **当前遗留 = 等用户 review 原型**；**真机 P-UI-02（1440 溢 32px / 1280 溢 192px）仍未修**，排期 U3。
  详见 04 §4.23 / 07 D24 / 08 §7 / 本文件 §3.1。
