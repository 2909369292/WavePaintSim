...
| 最近完成文档 | **第十一轮规划重排**（2026-09-09，纯文档）：近期主线改 #86 → #76 → #87①/②，#87 优先级提高，侧栏/整体 UI 重构设计提级，#84/#77 推迟远期（见 04 §4.9 / 08-ROADMAP §1~§3） |
...
| 当前阻塞 | 无 |
| 下一步 | ✅ #85 已收官（第十轮）。第十一轮重排后的近期主线 = **#86 → #76 → #87①/②**（重点：底层支持更多 Verilog 代码互动 + 代码解析能力；交互层仿 Verdi；#84/#77 已推迟远期）。现在开工 **#86**（实例→模块定义源码跳转 + 源码文件导入 + 例化解析增强，拆解 A1~A4 见 08 §1.1）。开工前先读 `08-ROADMAP.md` §1、`03-REQUIREMENTS.md` 表 H、04 §4.9 与 `memory/logs/2026-09-09.md` 第十一轮（重排口径）+ 第五轮（Verdi 语义规划）+ 第十轮（#85） |
...
> §1.2（#76 B1~B5）；台账 03 表 H；04 §4.5/§4.9。
...
1. 点变量 → 加波形（复用 `toNativeSignal` + `simWatches` 链路）。
...
3. **#76 B1 → B2/B4 → B3/B5**：#76 是 #86 的“近亲”——B1 符号索引/scope 映射直接复用
   A1 的解析底座；RTL 树点行加波形（B2）复用 #85 `pickVcdSignalIntoWave` 口径；代码内点
   变量加波形（B4 = #87①“中追”式）与 B2 走同一注入实现；#87②（模块全接口 Ctrl+4）紧跟。
