...
|---|---|
| 主线 | `main` |
| 当前版本 | 真机 `v0.4.0 build 2026-09-13 16:52:06 83fab85`（`83fab85` = **构建时 HEAD**，承载第二十五轮代码的 commit 是它的下一个，勿误判落后；**第二十七轮未改真机一行 → 数字不变**）；原型 `v0.4.0-mock build 2026-09-13 18:55:12 b93ad07`（**1,709,056 B**） |
| 最近完成 | **第二十七轮（2026-09-13）= 拖拽两条真 bug 修复（同一根因 · 06 P39 / 07 D27）+ 原型「1:1 完全照搬真机」重构（07 D26）+ 审计口径校准 + exe 重打包**：用户原话「目前各个面板的**拖动仍然有问题，拖动的预览和最后实际的效果不一致**，且**拖动按钮在拖动时的显示也有 bug**」「代码显示区和菜单栏的显示等**并没有照搬原本的源码**……我要 **1:1 完全一样的效果**，完全一致的效果**起码波形显示区是这样的**」。① 拖拽 bug 根因 = **闭包缓存 DOM 引用，被中途 `render()` 重建后变游离节点**（`elConnected=false`）→ 三条铁律修掉（稳定 id + 现查活节点 + 监听挂 window）+ I1~I5 回归；② 新增 `tools/gen-mock-page.mjs`（真机 `index.html` **逐字生成**原型页）+ `prototype/mock-tail.html` → **菜单栏/工具带/代码区/波形区 = 真机原样 DOM+CSS+JS**（13 same / 1 diff，波形画布像素 diff **0.0141%**）；③ `ui-audit --mock` 按 D26 豁免真机原生规则 → 两档违规 0；④ exe 内嵌 `prototype/ img/ css/ js/ lib/`（res 27→54）→ **1,709,056 B**，冒烟 **56/56**。**未触发 C1 / 真机 exe 一行未动**。详见 04 §4.25 / 本文件 §3.1 |
| 最近完成文档 | **第二十七轮记忆同步**（2026-09-13）：`04-PROGRESS.md` **新增 §4.25**（第二十七轮全记录：拖拽根因与三条铁律 / I1~I5 & H/G 回归 / 1:1 生成器与比对表 / 像素 diff / 审计口径 / exe 重打包与 `CS0016` 坑 / 实测汇总 / 改动文件表 / 红线）+ §1 快照切第二十七轮 + §2 时间线新增行；`09-HANDOFF.md` §1 快照 + **新增 §3.1（原 3.1~3.8 顺延为 3.2~3.9）**；`07-DECISIONS.md` 新增 **D26 + D27**；`06-PITFALLS.md` 新增 **P39**；`05-LOGS.md` 索引 + 当日日志 `logs/2026-09-13.md`；`INDEX.md` 页脚。**叠加生效**：原型 review = 双击 `WavePaintMockup.exe`；**原型页由 `tools/gen-mock-page.mjs` 生成（1:1 真机）** |
| 测试基线 | regression **79/79**；e2e-rtl **61/61**（#85 D1~D6 + 第十二轮 B3 + 第十四轮 E1~E6 + 第十五轮 F1~F9 + 第十六轮 G1~G7 + 第十七轮 H1/H2/H3/H4/H4b/H5/H6 + 第十八轮 I1~I6 + 第十九轮 J0~J7）；**e2e-ui 88/88**（真实 Edge；I0~I8 + 第二十五轮 I1 改口径 + 新增 I4c 指针拖拽）；e2e-sim 0 失败；probe-param 全过；真 exe 冒烟通过；**原型侧（第二十七轮更新）= `node tools/mockup-exe-smoke.mjs` 56/56 + `node tools/mock-probe.mjs` 55/55（dev-server 与 exe 服务双跑）+ `ui-audit --mock` 两档违规 0（D26 口径）+ 真机 vs 原型 13 same/1 diff + 像素 diff 波形 0.0141%** |
| 当前阻塞 | 无（U1 闸门已拍板 → **U0-R 顶栏统一**；**第二十七轮用户再明确「1:1 完全照搬真机」优先（D26）**；`08 §7.9-③④` 用户未提异议，按默认执行）。当前待办 = **等用户 review 第二十七轮 `WavePaintMockup.exe`**（1:1 真机界面 + 拖拽修复） |
| 下一步 | **① 等用户 review `WavePaintMockup.exe`**（双击 `D:\Files\Code\波形\WavePaintMockup.exe` → Edge 无地址栏窗口打开**1:1 真机界面**的原型，顶部绿条显示构建戳；关窗约 12s 后进程自动退出；**零依赖**）。⚠ **原型页由 `node tools/gen-mock-page.mjs` 从真机 `index.html` 生成**：改真机骨架后**必须先重跑生成器、再 `.\build-prototype.ps1`**。**② 未提异议即按 `08 §7.8` 总排期推进**（U 线与 D 线合并；**D-UI-A 已作废**：真机 21 个图标按钮**不下沉**、全留 `#toolbar`，U3 只做分组 / 刻度 / 尺寸 / `⋯` 溢出）：**U2 工具带 spec 数据驱动化**（`js/sim/ui/toolbar-spec.js`，24 个冻结 id 不改名；触发 C1 重建 exe）与 **D0 面板注册表 + 宿主容器抽象**（像素级零视觉变化）**合并同批** → **U3 真机落地 + 修 P-UI-02**（真机 1440 溢 32px / 1280 溢 192px）→ **D1 停靠引擎（必须遵守 D27：禁止缓存 DOM 引用，一律稳定 id + 现查活节点 + 监听挂 window）** → D2 页内浮动 → D3 持久化（不写 `.wp`）→ D4+U4。**#95（G1~G8）用户明确暂缓**；**#84 / #77 / #78 远期**；RTL 树只做代码层级浏览；服务自愈（C19 不变量）为独立专项。**每批次都要 C1 重建 exe + C8 核验 + 全量测试 + C17 记忆同步** |
...
### 3.1 第二十七轮（**当前生效**，2026-09-13）：**拖拽两条真 bug 修复（同一根因 · 06 P39 / 07 D27）+ 原型「1:1 完全照搬真机」重构（07 D26）+ 审计口径校准 + exe 重打包**
...
3. 在面板查看版本号，确认不是旧副本（本批应显示 `v0.4.0 build 2026-09-13 16:52:06 83fab85`；
   ⚠ `83fab85` = **构建时 HEAD**（第二十四轮的 commit），承载第二十五轮代码的 commit 是它的**下一个**，不代表落后）。
   内置第二十五轮 `js/sim/panel-layout.js` splitter 基线模型重写 + 混淆残留清零、第二十四轮 U 线原型、
   第二十一轮可拖拽多面板、第十九轮 #87②（`Ctrl+Alt+4`）、第十八轮 #76 B5、第十七轮 #76 B3、
   第十六轮 #76 B4、第十五轮 #76 B1、第十四轮 #86 A1~A4、第十三轮服务自愈、第十二轮 RTL 树瘦身、
   #85 观察行链路。
4. **原型 review 交付物（第二十六轮新增，与真机互不影响）**：双击 `D:\Files\Code\波形\WavePaintMockup.exe`
   → Edge 无地址栏窗口打开 U0-R 原型（**纯 mock / 零后端 / 假数据**），窗口顶部绿条显示
   `v0.4.0-mock build 2026-09-13 17:08:12 2d1636a`；关窗约 12 秒后进程自动退出；**不需要 Node、不需要命令**。
   真机 `WavePaintClean.exe` 不受影响（端口 17817 / 17820、身份串 `WAVEPAINT-SERVICE` / `WAVEPAINT-MOCKUP` 分离）。
...
5. 若此前桌面/任务栏堆积了多个「WavePaint 波形编辑」窗口，先全部关掉，再确认没有
